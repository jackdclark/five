...
assert.equal(
  ...
  five.law(),
  "The Law of Fives should never be wrong"
);
...
assert.equal(
  "wǔ",
  five.chinese("pinyin"),
  "A chinese five in Pīnyīn should be wǔ"
);
assert.equal(
  "伍",
  five.chinese("financial"),
  "A chinese five in financial contexts should be 伍"
);
...
assert.equal(
  "lempë",
  five.elvish("quenya"),
  "An elvish five in Quenya should be lempë"
);
assert.equal(
  "leben",
  five.elvish("sindarin"),
  "An elvish five in Sindarin should be leben"
);
...
assert.equal(
  ...
  five.mdFive(),
  'md5 checksum of "five" should be 30056e1cab7a61d256fc8edd970d14f5'
);
...
assert.equal(
  "IVEFAY",
  five.loud("piglatin"),
  "A loud five in Pig Latin should be IVEFAY"
);
assert.equal(
  "ПЯТЬ",
  five.loud("russian"),
  "A loud five in Russian should be ПЯТЬ"
);
...
assert.equal(
  "🕔",
  five.oclock(),
  "A unicode symbol for five o'clock should be U+1F554"
);
assert.equal(
  "🍺",
  five.oclockSomewhere(),
  "A unicode symbol for 'It's five o'clock somewhere' should be U+1F37A"
);
assert.equal(
  "🍔",
  five.guys(),
  "A unicode symbol for Five Guys should be U+1F354"
);
...
assert.ok(
  finishes - now > 500,
  "A too slow five should take longer than 500 milliseconds to be returned, blocking execution and generally being a bad idea"
);

assert.equal(
  JSON.stringify(["Jackie", "Tito", "Jermaine", "Marlon", "Michael"]),
  JSON.stringify(five.jackson()),
  "A Jackson five should be the five members of the 70's pop family group the Jackson 5"
);
...
assert.equal(
  ...
  JSON.stringify(five.fab()),
  "A fab five should be the 1991-1993 Michigan Mens Basketball Team"
);
...
assert.equal(
  ...
  JSON.stringify(five.members()),
  "A five five should be the 90's boy band 5ive"
);

assert.equal(
  JSON.stringify(["5ive", "Invincible", "Kingsize"]),
  JSON.stringify(five.discography()),
  "A five discography should be the studio albums released by the 90's boy band 5ive"
);
...
assert.equal(
  ...
  JSON.stringify(five.singles()),
  "A five singles should be the singles released by the 90's boy band 5ive"
);

assert.equal(
  JSON.stringify(["Tigress", "Viper", "Crane", "Monkey", "Mantis"]),
  JSON.stringify(five.furious()),
  "A Furious five should be the five fictional members of the ninja group Furious Five from the movie Kung-Fu Panda"
);

assert.equal(
  JSON.stringify(["Julian", "Dick", "George", "Anne", "Timmy"]),
  JSON.stringify(five.famous()),
  "A Famous five should be the five central characters of the series of Enid Blighton novels"
);

assert.equal(
  "Interstella 5555: The 5tory of the 5ecret 5tar 5ystem",
  five.fiveFiveFive(),
  "FiveFiveFiveFive should be Interstella 5555: The 5tory of the 5ecret 5tar 5ystem"
);
...
assert.equal(
  ...
  "A funked five should be a group of bad boys with the power to rock you"
);
...
assert.equal(
  ...
  five.rot("abcdefghijklmnopqrstuvwxyz"),
  "Small letters should be rotated"
);
assert.equal(
  "FGHIJKLMNOPQRSTUVWXYZABCDE",
  five.rot("ABCDEFGHIJKLMNOPQRSTUVWXYZ"),
...
assert.equal(
  "$_$ -,- @.@?",
  five.rot("$_$ -,- @.@?"),
  "Emoticons should not be rotated"
);
...
setTimeout(function () {
  if (!emitterTested) {
    console.error(
      "The five event emitter did something unexpected! I hope you're not using this feature in production"
    );
    process.exit(1);
  }
  process.exit(0);
